...
export class PdfService {
  ...
  async renderPage(pageNumber: number, canvas: HTMLCanvasElement, scale: number = 1.5): Promise<{ width: number; height: number }> {
    if (!this.pdfDoc) {
  ...
  async renderThumbnail(pageNumber: number, canvas: HTMLCanvasElement, maxWidth: number = 150): Promise<void> {
    if (!this.pdfDoc) {
      throw new Error('PDF não carregado');
    }
  ...
  downloadPdf(pdfBytes: Uint8Array, filename: string = 'documento-editado.pdf'): void {
